...
1. Copy over the dependencies and the npm tasks from [package.json](https://github.com/roxiness/routify-starter/blob/master/package.json).

2. Copy over the full content from [rollup.config.js](https://github.com/roxiness/routify-starter/blob/master/rollup.config.js)
...
If you are looking to deploy your project to Now or Netlify, look in the [scripts folder](https://github.com/roxiness/routify-starter/tree/master/scripts) and copy the files you need.
...
1. Copy over the dependencies and the npm tasks from routify-starter's [package.json](https://github.com/roxiness/routify-starter/blob/master/package.json).

2. Copy over the full content from routify-starter's  [rollup.config.js](https://github.com/roxiness/routify-starter/blob/master/rollup.config.js)

3. Delete the following files in `src`: `client.js`, `server.js`

4. Rename the `routes` folder to `pages`

5. Move `template.html` to `static/__index.html`. You will have to remove the specific things like marked with `%sapper%`. Make any changes you feel necessary to correspond as much as possible to [this file](https://github.com/roxiness/routify-starter/blob/master/static/__index.html) - most notably add the `__SCRIPT__` marker somewhere in your `<head>` section.
...
Remove `_error.svelte` and replace it with [`_fallback.svelte`](https://github.com/roxiness/routify-starter/blob/master/src/pages/_fallback.svelte).
